...
(deftest init
  ...
  (testing "Fetching the last dup for a key"
    (let [e (env "/tmp"
      ...
      (is (= 3 (-> (last-dup e :dups (in 1)) (out))))
      (with-txn [txn (read-txn e)]
        (is (= 5 (-> (last-dup e :dups txn (in 2)) (out))))
        (is (= 15 (-> (last-dup e :dups txn (in 8)) (out)))))
      (is (= 0 (-> (last-dup e :dups (in 0)) (out))))
      (is (= 16 (-> (last-dup e :dups (in 9)) (out))))
      (cleanup))))
